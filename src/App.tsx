...
type View = 'dashboard' | 'entry' | 'history' | 'report';
...
function App() {
  ...
  const latestRound = sortedHistory[0] ?? round;
  const latestMetrics = getMetrics(latestRound);
  ...
  const stats: Stat[] = [
    {
      label: '最近总杆',
      value: `${latestMetrics.totalStrokes}`,
      tone: latestMetrics.totalStrokes < 80 ? 'good' : latestMetrics.totalStrokes <= 86 ? 'warn' : 'bad',
    },
    { label: '平均杆数', value: averageStats[0].value },
    { label: '平均推杆', value: averageStats[1].value },
    { label: 'GIR', value: percent(latestMetrics.girCount, 18), tone: latestMetrics.girCount >= 9 ? 'good' : latestMetrics.girCount >= 6 ? 'warn' : 'bad' },
    { label: '球道命中', value: percent(latestMetrics.fairwayHits, latestMetrics.fairwayTotal), tone: latestMetrics.fairwayHits >= 8 ? 'good' : latestMetrics.fairwayHits >= 6 ? 'warn' : 'bad' },
    { label: 'OB 次数', value: `${latestMetrics.obCount}`, tone: latestMetrics.obCount === 0 ? 'good' : 'bad' },
  ];
  ...
  const openReport = (roundId?: string) => {
    if (roundId) setSelectedReportRoundId(roundId);
    setView('report');
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  return (
    <main className="app-shell">
      <AppHeader view={view} onSave={saveRound} onReset={resetRound} />
      ...
      {view === 'history' && (
        <HistoryView
          ...
          loadHistoryRound={loadHistoryRound}
      ...
      {view === 'report' && (
        <ReportView
          ...
          report={report}
          topThree={topThree}
          onEdit={(savedRound) => loadHistoryRound(savedRound)}
...
function AppHeader({ view, onSave, onReset }: { view: View; onSave: () => void; onReset: () => void }) {
  ...
  return (
    <header className="app-header">
      <div>
        <p className="eyebrow">{titleMap[view]}</p>
        <h1>Golf 80 Breaker</h1>
        <p className="header-subtitle">80杆突破训练系统</p>
      </div>
      {view === 'entry' && (
        <div className="header-actions">
          <button className="icon-button primary" type="button" onClick={onSave} aria-label="保存本轮">
            <Save size={18} />
          </button>
          <button className="icon-button" type="button" onClick={onReset} aria-label="新一轮">
            <PencilLine size={18} />
          </button>
        </div>
      )}
...
function DashboardView({
  ...
  stats: Stat[];
  recentFive: SavedRound[];
  historyCount: number;
  latestMetrics: Metrics;
  targetAnalysis: {
    targetSaving: number;
    opportunities: TargetOpportunity[];
    ranked: TargetOpportunity[];
  ...
  return (
    <div className="view-stack">
      <section className="hero-card">
        <div>
          <p>最近一轮</p>
          <strong>{latestMetrics.totalStrokes}</strong>
          <span>{latestMetrics.scoreTo80 > 0 ? `距离 79 还差 ${latestMetrics.scoreTo80} 杆` : '已进入 80 杆以内'}</span>
        </div>
      ...
      <section className="metric-grid">
        {stats.map((stat, index) => (
          <MetricCard key={stat.label} stat={stat} iconIndex={index} />
        ))}
      </section>

      <section className="surface-card">
        <SectionTitle title="最近 5 轮趋势" meta={historyCount > 0 ? `共 ${historyCount} 轮` : '还没有历史'} />
        <ScoreTrendChart rounds={recentFive} />
      </section>

      <section className="surface-card">
        <SectionTitle title="80 杆以内目标分析" meta={`86 → 79：少 ${targetAnalysis.targetSaving} 杆`} />
        <div className="target-summary">
          <Target size={18} />
          <p>优先寻找不靠爆发也能省的杆数：少罚杆、少三推、推杆压到 32、GIR 提到 8 个以上。</p>
        </div>
      ...
      <section className="surface-card">
        <SectionTitle title="当前最大短板" meta="按失分排序" />
        <div className="issue-list">
          {lossCategories.slice(0, 3).map((category) => (
            <article className={`issue-card ${scoreTone(category.lostShots)}`} key={category.id}>
              <div>
        ...
        <button className="wide-button" type="button" onClick={() => setView('report')}>
          查看复盘建议
...
function EntryView({
  ...
  round: RoundDraft;
  metrics: Metrics;
  setRound: (round: RoundDraft) => void;
  updateHole: <Key extends keyof Hole>(index: number, key: Key, value: Hole[Key]) => void;
  onSave: () => void;
  onReset: () => void;
}) {
  return (
    <div className="view-stack">
      <section className="surface-card">
        <SectionTitle title="本轮信息" meta="自动保存草稿" />
        <div className="round-meta">
          ...
          <label>
            ...
            <input
              type="text"
              value={round.course}
              placeholder="例如：XX 球会"
              onChange={(event) => setRound({ ...round, course: event.target.value })}
            />
          </label>
        </div>
        <div className="entry-summary">
          <span>总杆 {metrics.totalStrokes}</span>
          <span>推杆 {metrics.totalPutts}</span>
          <span>GIR {percent(metrics.girCount, 18)}</span>
        </div>
      ...
      <section className="holes-list">
        {round.holes.map((hole, index) => (
          <HoleCard key={index} hole={hole} index={index} updateHole={updateHole} />
        ))}
      </section>
    </div>
  );
...
function HistoryView({
  ...
  sortedHistory: SavedRound[];
  selectedRound?: SavedRound;
  selectedMetrics?: Metrics;
  setSelectedRoundId: (id: string) => void;
  loadHistoryRound: (round: SavedRound) => void;
  ...
  if (sortedHistory.length === 0) {
    return (
      <section className="empty-panel">
        ...
        <h2>还没有历史记录</h2>
        <p>保存一轮成绩后，这里会显示日期、总杆、推杆、GIR 和球道命中率。</p>
      </section>
    );
  }
    ...
    <div className="view-stack">
      <section className="history-list">
        {sortedHistory.map((item) => {
          ...
          return (
            <button
              className={`round-list-card ${active ? 'active' : ''}`}
              key={item.id}
              type="button"
            ...
            >
              <div>
                ...
                <span>{item.course || '未命名球场'}</span>
              </div>
              <div className="round-mini-stats">
                <b>{itemMetrics.totalStrokes}</b>
                <span>{itemMetrics.totalPutts} 推</span>
                <span>GIR {percent(itemMetrics.girCount, 18)}</span>
                <span>球道 {percent(itemMetrics.fairwayHits, itemMetrics.fairwayTotal)}</span>
              </div>
            </button>
          );
        })}
      ...
      {selectedRound && selectedMetrics && (
        <section className="surface-card">
          <SectionTitle title={selectedRound.course || '未命名球场'} meta={selectedRound.date} />
          ...
          <div className="history-actions">
            <button className="primary-button" type="button" onClick={() => openReport(selectedRound.id)}>
              查看复盘
            </button>
            <button className="secondary-button" type="button" onClick={() => loadHistoryRound(selectedRound)}>
              载入编辑
            </button>
          </div>
          <div className="hole-detail-grid">
            {selectedRound.holes.map((hole, index) => (
              <div className="hole-detail" key={`${selectedRound.id}-${index}`}>
                ...
                <span>Par {hole.par} / {hole.strokes} 杆 / {hole.putts} 推</span>
                <small>
                  {hole.gir ? 'GIR' : '未 GIR'}
...
function ReportView({
  ...
  reportRound?: SavedRound;
  report?: RoundReview;
  topThree: Array<{ title: string; body: string }>;
  onEdit: (round: SavedRound) => void;
  onEntry: () => void;
}) {
  if (!reportRound || !report) {
    return (
      <section className="empty-panel">
        <ClipboardList size={34} />
        <h2>还没有复盘报告</h2>
        <p>保存一轮成绩后，系统会自动生成单轮复盘。</p>
        <button className="primary-button" type="button" onClick={onEntry}>
          去录入
        </button>
      </section>
    );
  ...
  return <RoundReviewReport round={reportRound} report={report} topThree={topThree} onEdit={() => onEdit(reportRound)} />;
}
...
function RoundReviewReport({
  ...
  return (
    <div className="view-stack">
      <section className={`report-hero ${gap > 6 ? 'danger' : gap > 0 ? 'warning' : 'success'}`}>
        <div>
          <p>{round.date}{round.course ? ` · ${round.course}` : ''}</p>
          <h2>{gap === 0 ? '达标' : `差 ${gap} 杆`}</h2>
          <span>{metrics.totalStrokes} 杆 / 目标 79</span>
        </div>
      ...
      <section className="report-grid">
        <ReportStat label="推杆" value={`${metrics.totalPutts}`} tone={metrics.totalPutts > 34 ? 'warn' : 'good'} />
        <ReportStat label="GIR" value={`${metrics.girCount}/18`} tone={metrics.girCount < 6 ? 'bad' : metrics.girCount < 8 ? 'warn' : 'good'} />
        <ReportStat label="球道" value={`${fairwayRate}%`} tone={fairwayRate < 45 ? 'bad' : fairwayRate < 60 ? 'warn' : 'good'} />
        <ReportStat label="OB / 罚杆" value={`${metrics.obCount} / ${metrics.penalties}`} tone={metrics.obCount > 1 || metrics.penalties > 2 ? 'bad' : 'good'} />
      </section>

      <section className="surface-card">
        <SectionTitle title="报告判断" meta="数据校准" />
        <div className="report-summary">
          <p>{bandText}</p>
          <p>最大失分来源是 <strong>{biggestLoss.title}</strong>：{biggestLoss.evidence}</p>
        </div>
      </section>

      <section className="surface-card">
        <SectionTitle title="最该练的 3 件事" meta="实战优先级" />
        <div className="review-list">
          {easiestSavings.map((item, index) => (
            <article className={`review-card ${scoreTone(Math.ceil(item.impact))}`} key={item.id}>
              <span>{index + 1}</span>
              <div>
                <h3>{item.title}</h3>
      ...
      <section className="surface-card">
        <SectionTitle title="下一轮 3 条策略" meta="比赛执行" />
      ...
      <section className="surface-card">
        <SectionTitle title="训练建议" meta="补充" />
...
function HoleCard({
  ...
  return (
    <article className="hole-card">
      <div className="hole-head">
        <div>
          <strong>{index + 1} 洞</strong>
          <span>Par {hole.par}</span>
        </div>
        <b>{hole.strokes - hole.par >= 0 ? '+' : ''}{hole.strokes - hole.par}</b>
      </div>

      <div className="field-grid">
        <NumberField label="Par" value={hole.par} min={3} max={5} onChange={(value) => updateHole(index, 'par', value)} />
        <NumberField label="Score" value={hole.strokes} min={1} max={12} onChange={(value) => updateHole(index, 'strokes', value)} />
        <NumberField label="Putts" value={hole.putts} min={0} max={6} onChange={(value) => updateHole(index, 'putts', value)} />
        <NumberField label="罚杆" value={hole.penalties} min={0} max={6} onChange={(value) => updateHole(index, 'penalties', value)} />
      </div>

      <div className="toggle-row">
        {hole.par > 3 && (
          <Toggle label="Fairway" checked={hole.fairway} onChange={(value) => updateHole(index, 'fairway', value)} />
        )}
        <Toggle label="GIR" checked={hole.gir} onChange={(value) => updateHole(index, 'gir', value)} />
        <Toggle label="OB" checked={hole.ob} onChange={(value) => updateHole(index, 'ob', value)} danger />
      </div>
...
function MetricCard({ stat, iconIndex }: { stat: Stat; iconIndex: number }) {
  const icons = [Gauge, Activity, Flag, Target, BarChart3, ListChecks];
  const Icon = icons[iconIndex] ?? Activity;

  return (
    <article className={`metric-card ${stat.tone ?? ''}`}>
...
function ReportStat({ label, value, tone }: { label: string; value: string; tone: 'good' | 'warn' | 'bad' }) {
  return (
...
function Toggle({
  ...
  return (
    <label className={`toggle ${checked ? 'checked' : ''} ${danger ? 'danger' : ''}`}>
...
function BottomNav({ view, setView }: { view: View; setView: (view: View) => void }) {
  const items: Array<{ id: View; label: string; icon: typeof Home }> = [
    ...
    { id: 'entry', label: '录入', icon: PencilLine },
    { id: 'history', label: '历史', icon: CalendarDays },
    { id: 'report', label: '复盘', icon: ClipboardList },
  ];
  ...
  return (
    <nav className="bottom-nav" aria-label="主导航">
      {items.map((item) => {
        ...
        return (
          <button className={active ? 'active' : ''} key={item.id} type="button" onClick={() => setView(item.id)}>
            <Icon size={20} />
